...
export default function Caretaker() {
  ...
  const [notifications] = useState([
    { id: 1, message: "New shift assigned for tomorrow 9 AM", time: "5 min ago", unread: true },
    { id: 2, message: "Patient John Smith requested care notes update", time: "1 hour ago", unread: true },
    { id: 3, message: "Weekly report submission due in 2 days", time: "3 hours ago", unread: false },
  ]);
  ...
  const [careNotes, setCareNotes] = useState([
    { id: 1, patient: "John Smith", date: "2025-10-09", time: "14:30", note: "Patient showed improvement in mobility. Completed physical therapy exercises with minimal assistance.", category: "Progress" },
    { id: 2, patient: "Mary Johnson", date: "2025-10-08", time: "10:15", note: "Post-surgery wound healing well. Patient in good spirits and following medication schedule.", category: "Medical" },
  ]);
...
const handleUserDataSave = async () => {
  ...
  const renderUserDetailsPage = () => (
    <div className="cont p-6 bg-sky-100 rounded-lg shadow-sm border">
      ...
      <form onSubmit={(e) => { e.preventDefault(); handleUserDataSave(); }} className="space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            <label>Full Name</label>
            <input type="text" value={caretakerData.name} onChange={(e) => setCaretakerData({ ...caretakerData, name: e.target.value })} className="w-full border px-4 py-2 rounded-lg" required />
          </div>
          <div>
            <label>Email</label>
            <input type="email" value={caretakerData.email} onChange={(e) => setCaretakerData({ ...caretakerData, email: e.target.value })} className="w-full border px-4 py-2 rounded-lg" required />
          </div>
          <div>
            <label>Phone</label>
        ...
        <div>
          <label>Address</label>
          <input type="text" value={caretakerData.address} onChange={(e) => setCaretakerData({ ...caretakerData, address: e.target.value })} className="w-full border px-4 py-2 rounded-lg" required />
        </div>
        <div>
          <label>Specialization</label>
          <textarea value={caretakerData.specialization} onChange={(e) => setCaretakerData({ ...caretakerData, specialization: e.target.value })} className="w-full border px-4 py-2 rounded-lg" rows={3} />
        </div>
        <div>
          <label>Emergency Contact</label>
          <input type="tel" value={caretakerData.emergencyContact} onChange={(e) => setCaretakerData({ ...caretakerData, emergencyContact: e.target.value })} className="w-full border px-4 py-2 rounded-lg" />
        </div>
...
const renderAvailabilityPage = () => (
  ...
  const renderReportsPage = () => (
    <div className="cont  ">
      ...
      {careNotes.map((note) => (
        <div key={note.id} className="mb-2 p-2 border rounded-lg bg-white shadow-sm">
          <p><strong>{note.patient}</strong> ({note.date} at {note.time})</p>
          <p>{note.note}</p>
        </div>
      ))}
    </div>
      ...
      <div className="flex">
        ...
        <div className="w-80 bg-sky-300 shadow-lg border-r p-4 flex flex-col">
          ...
          <div className="flex justify-center space-x-6 mb-5">
            ...
            <button onClick={() => setActivePage("userDetails")} className="w-12 h-12 bg-green-100 rounded-full flex items-center justify-center hover:bg-green-200 transition-colors">
          ...
          <nav className="space-y-2 mt-3">
            {[
              ...
              { id: "assignedPatients", label: "Assigned Patients", icon: "🏥" },
              { id: "availability", label: "Availability Toggle", icon: "⏰" },
              { id: "reports", label: "Daily Reports & Care Notes", icon: "📋" },
            ].map((item) => (
              <button
                key={item.id}
