...
export async function getAssignedPatients(req, res) {
  try {
    ...
    // The frontend sends localStorage "userId" (User._id), so look up the Caretaker linked to that user
    ...
    const schedules = await Schedule.find({ caretakerId: caretaker._id })
      .populate('patientId', 'patientId name age phone address emergencyContact notes')
      .sort({ startDate: -1 })
      .lean()
