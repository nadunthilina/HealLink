...
router.get(
  "/assigned-patients/:caretakerId",
  requireAuth(["caretaker", "admin"]),
  getAssignedPatients,
);
...
async function checkCaretakerConflict(
  caretakerId,
  startDate,
  startTime,
  dayType,
  ...
  for (const existing of existingSchedules) {
    ...
    const existingEnd = getEndDateTime(
...
router.get("/my", requireAuth(["patient"]), async (req, res) => {
  try {
    ...
    const schedules = await Schedule.find({ patientId: patient._id })
      ...
      .populate(
        "caretakerId",
        "caretakerId name phone gender skills availability",
      )
  ...
  try {
    ...
    if (
      ...
      return res.status(400).json({
        message:
          "Patient, Caretaker, Start Date, Start Time, and Day Type are required",
      });
    }
    ...
    for (const date of dates) {
      ...
      if (conflict) {
        return res
          .status(409)
          .json({ message: `Conflict on ${date}: ${conflict.message}` });
      }
    }
    ...
    const finalPaymentToAgency =
      req.user.role === "patient" ? "unpaid" : paymentToAgency || "unpaid";
    const finalStatus =
      req.user.role === "patient" ? "pending" : status || "pending";
  ...
  async (req, res) => {
    try {
      ...
      if (schedule.patientId.toString() !== patient._id.toString()) {
        return res
          .status(403)
          .json({ message: "Access denied to this schedule" });
      }
      if (schedule.jobCompletedByPatient) {
        return res.json({
          message: "Schedule already marked complete",
          schedule,
        });
      }
      schedule.jobCompletedByPatient = true;
      if (
        schedule.jobCompletedByAdmin &&
        schedule.paymentToAgency === "paid" &&
        schedule.paymentToCaretaker === "success"
      ) {
...
router.put("/:id", requireAuth(["admin"]), async (req, res) => {
  try {
    ...
    const isCoreEdit =
      existingSchedule.caretakerId.toString() !== safeCaretakerId.toString() ||
      new Date(existingSchedule.startDate).toISOString().split("T")[0] !==
        new Date(safeStartDate).toISOString().split("T")[0] ||
    ...
    if (existingSchedule.status !== "pending" && isCoreEdit) {
      return res.status(400).json({
        message:
          "Cannot edit core details of a schedule that has already started or completed.",
      });
    }
    if (isCoreEdit) {
      const conflict = await checkCaretakerConflict(
    ...
    let newPaymentToAgency =
      paymentToAgency || existingSchedule.paymentToAgency;
    let newPaymentToCaretaker =
      paymentToCaretaker || existingSchedule.paymentToCaretaker;
    if (
      (jobCompletedByPatient ||
        jobCompletedByAdmin ||
        existingSchedule.jobCompletedByPatient ||
        existingSchedule.jobCompletedByAdmin) &&
      newPaymentToAgency === "paid" &&
      newPaymentToCaretaker === "success"
    ) {
      newStatus = "completed";
    }
    ...
    const schedule = await Schedule.findByIdAndUpdate(
      req.params.id,
      {
        ...
        jobCompletedByPatient:
          jobCompletedByPatient !== undefined
            ? jobCompletedByPatient
            : existingSchedule.jobCompletedByPatient,
        jobCompletedByAdmin:
          jobCompletedByAdmin !== undefined
            ? jobCompletedByAdmin
            : existingSchedule.jobCompletedByAdmin,
        adminNote:
          adminNote !== undefined ? adminNote : existingSchedule.adminNote,
...
router.delete("/:id", requireAuth(["admin"]), async (req, res) => {
  try {
    ...
    if (!existingSchedule)
      return res.status(404).json({ message: "Schedule not found" });
    if (
      existingSchedule.status === "start" ||
      existingSchedule.status === "completed"
    ) {
      return res.status(400).json({
        message: "Cannot delete a schedule that has started or completed",
      });
    }
